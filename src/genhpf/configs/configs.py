...
class DistributedTrainingConfig(BaseConfig):
    ...
    distributed_port: int = field(
        default=12355,
        metadata={"help": "port number for distributed training"}
    )
    device_id: int = field(
        default=0,
    ...
    find_unused_parameters: bool = field(
        default=False,
        metadata={
            "help": "disable unused parameter detection when using distributed training"
        }
    )
...
class DatasetConfig(BaseConfig):
    data_format: str = field(
        default="genhpf",
        metadata={"help": "data format. supported formats: genhpf, meds"}
    )
    data: str = field(
        default=MISSING,
    ...
    label: bool = field(
        default=False,
        metadata={"help": "whether to load labels from the dataset"}
    )
    ...
    sep_token_id: int = field(
        default=102,
        metadata={"help": "sep token id"}
    )
    dummy_token_id: int = field(
        default=101,
        metadata={"help": "dummy token id"}
    )
    ...
    apply_mask: bool = field(
        default=False,
        metadata={"help": "whether to apply masking to the input tokens"}
    )
    mask_prob: float = field(
        default=0.15,
        metadata={"help": "probability for masking tokens"}
    )
    mask_unit: str = field(
        default="token",
        metadata={"help": "unit for masking. supported units: token, event"}
    )
    ...
    train_subset: str = field(
        default="train",
        metadata={"help": "data subset name to use for training (e.g., train, valid, test)"}
    )
    valid_subset: str = field(
        default="valid",
        metadata={"help": "comma separated list of data subset names to use for validation"}
    )
    test_subset: str = field(
        default="test",
        metadata={"help": "comma separated list of data subset names to use for test"}
    )
    ...
    disable_validation: Optional[bool] = field(
        default=None,
        metadata={"help": "whether to disable validation during training"}
    )

@dataclass
class OptimizationConfig(BaseConfig):
    max_epoch: int = field(
        default=0, metadata={"help": "maximum number of epochs to train"}
    )
    lr: float = field(
    ...
    weight_decay: float = field(
        default=1e-8, metadata={"help": "weight decay for Adam optimizer"}
    )
...
class CheckpointConfig(BaseConfig):
    ...
    load_checkpoint: Optional[str] = field(
        default=None,
        metadata={"help": "path to a checkpoint to load model weights from, if provided"}
    )
    no_save: bool = field(
        default=False, metadata={"help": "don't save checkpoints"}
    )
