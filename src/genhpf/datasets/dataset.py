...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def mask(
        ...
        if mask_unit == "token":
            ...
            mask_indices = torch.zeros_like(tokens).bool()
            masked_event_indices = (
                torch.randperm(len(event_indices) - 1)[:round((len(event_indices) - 1) * mask_prob)]
            )
        ...
        mask_positions = torch.bernoulli(torch.full(labels.shape, 0.8)).bool() & mask_indices
        tokens[0, mask_positions] = mask_token_idx # for input_ids
        ...
        random_positions = (
            torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & mask_indices & ~mask_positions
        )
        random_words = torch.randint(vocab_size, labels.shape, dtype=torch.long)
        tokens[0, random_positions] = random_words[random_positions]
        random_types = torch.randint(7, labels.shape, dtype=torch.long)
        tokens[1, random_positions] = random_types[random_positions]
        random_dpes = torch.randint(16, labels.shape, dtype=torch.long)
